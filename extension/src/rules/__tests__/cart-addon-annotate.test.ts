 ...
 */
import { CART_ADDON_ANNOTATED_ATTR as FLAGGED_ATTR } from "../../lib/dom-markers";
import { cartAddonAnnotateRule, matchAddon } from "../cart-addon-annotate";
...
afterEach(() => {
  cartAddonAnnotateRule.teardown();
  jest.useRealTimers();
});
...
describe("cartAddonAnnotateRule on checkout URLs", () => {
  it("annotates a protection-plan line item", () => {
    ...
    `;
    cartAddonAnnotateRule.apply(document.body);
