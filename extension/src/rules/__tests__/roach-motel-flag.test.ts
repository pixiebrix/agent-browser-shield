...
import { findWarning, roachMotelFlagRule } from "../roach-motel-flag";

const LANDMARK_SELECTOR = 'section[data-abs-rule="roach-motel-flag"]';
...
describe("roachMotelFlagRule.apply (on nytimes.com/subscription/all-access)", () => {
  it("prepends an sr-only landmark with the NYT notice", () => {
    document.body.innerHTML = `<main id="main">page content</main>`;
    roachMotelFlagRule.apply(document.body);
