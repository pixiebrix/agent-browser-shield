...
- `svg-text-strip` — blank injection-shaped text inside in-page SVG
  `<title>`, `<desc>`, and `<text>` elements (covers logos, infographics, and
  charts; complements `svg-sprite-strip`, which only removes unused sprite
  containers)
